...
def send_job_listings_to_codeforcash(listings):
    ...
    # fhand = urllib.request.urlopen(data_send_job_lisitings_to_codeforcash["website"])
        ...
        try:
            html_request = urllib.request.urlopen(data_of_each_listing["website"]).read()
        except urllib.error.HTTPError as e:
            print(e)
        else:
            target_class_info = SoupStrainer("div", {"class" : "section-wrapper page-full-width"}) 
            soup = BeautifulSoup(html_request, "html.parser", parse_only=target_class_info)
